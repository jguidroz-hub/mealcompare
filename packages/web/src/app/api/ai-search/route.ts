 ...
 * 
 * Parses natural language intent and returns ranked results
 */
...
function scoreRestaurant(r: any, intent: ParsedIntent): number {
  ...
  // Has direct ordering (strong boost)
  if (r.directUrl) score += 25;

  // Mood-based category matching
...
export async function POST(req: NextRequest) {
  try {
    ...
    // Generate a human-friendly interpretation
    const parts: string[] = [];
    ...
    return NextResponse.json({
      ...
      metro,
      intent: {
        understood: parts.join(' · ') || 'general search',
