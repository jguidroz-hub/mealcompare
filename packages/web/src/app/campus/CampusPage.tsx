import RefCookie from '../ut/RefCookie';
...
export interface CampusConfig {
  ...
  city: string;
  tagline: string;
  restaurants: { name: string; category: string; savings: string }[];
  totalRestaurants: number;
  cityRestaurants: number;
...
export default function CampusPage({ config }: { config: CampusConfig }) {
  const semesterSavings = config.ordersPerWeek * config.avgSavings * config.weeksPerSemester;
  ...
  const jsonLd = {
    ...
    description: `Free Chrome extension that compares prices across food delivery apps and finds cheaper direct ordering options for ${config.schoolName} students.`,
  };
      ...
      <main style={{
        ...
        <section style={{
          ...
          <div style={{
            ...
            {config.emoji} Built for {config.schoolName} students
          </div>
            ...
            {[
              ...
              <div key={item.step} style={{
                ...
                <div style={{
                  ...
                  background: '#f97316', color: '#fff', fontWeight: 800, fontSize: '18px',
        ...
        <section style={{ maxWidth: '800px', margin: '0 auto', padding: '40px 24px' }}>
          <div style={{
            ...
            <h2 style={{ fontSize: '22px', fontWeight: 700, marginBottom: '24px' }}>The math is simple</h2>
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '16px', marginBottom: '24px' }}>
              <div>
                <div style={{ fontSize: '32px', fontWeight: 800, color: '#f97316' }}>{config.ordersPerWeek}×</div>
                <div style={{ fontSize: '13px', color: '#94a3b8' }}>orders per week</div>
              </div>
              <div>
                <div style={{ fontSize: '32px', fontWeight: 800, color: '#f97316' }}>${config.avgSavings}</div>
        ...
        <section style={{ maxWidth: '800px', margin: '0 auto', padding: '40px 24px' }}>
          <h2 style={{ fontSize: '24px', fontWeight: 700, textAlign: 'center', marginBottom: '8px' }}>
            {config.totalRestaurants}+ restaurants near campus
          </h2>
          ...
          <p style={{ textAlign: 'center', color: '#64748b', fontSize: '13px', marginTop: '16px' }}>
            ...and {config.cityRestaurants - config.restaurants.length}+ more. Eddy works on any restaurant page on DoorDash, Uber Eats, or Grubhub.
          </p>

          <div style={{
            marginTop: '40px', background: '#0f172a',
            ...
            <a
              href={`mailto:${config.feedbackEmail}?subject=Eddy%20${config.shortName}%20Feature%20Request`}
              style={{
        ...
        {/* FAQ */}
        <section style={{ maxWidth: '800px', margin: '0 auto', padding: '40px 24px' }}>
          <h2 style={{ fontSize: '24px', fontWeight: 700, textAlign: 'center', marginBottom: '24px' }}>FAQ</h2>
          {[
            ...
            { q: 'Does it work for all restaurants?', a: `Eddy covers ${config.totalRestaurants}+ restaurants near ${config.shortName} and ${config.cityRestaurants}+ across ${config.city}. If a restaurant has its own ordering system, Eddy will find it.` },
            ...
            { q: 'How much will I actually save?', a: `Delivery apps mark up prices 15-30% and add service fees. Most students save $3-8 per order. At ${config.ordersPerWeek} orders/week, that's $${semesterSavings}/semester.` },
          ].map((item) => (
            <details key={item.q} style={{
              ...
              <summary style={{ fontWeight: 600, fontSize: '15px', listStyle: 'none' }}>{item.q}</summary>
              <p style={{ fontSize: '14px', color: '#94a3b8', marginTop: '12px', lineHeight: 1.6 }}>{item.a}</p>
            </details>
          ))}
        </section>
        ...
        <section style={{ maxWidth: '800px', margin: '0 auto', padding: '60px 24px', textAlign: 'center' }}>
          <h2 style={{ fontSize: '28px', fontWeight: 800, marginBottom: '16px' }}>Your wallet will thank you</h2>
          ...
          <a
            ...
            style={{
              ...
              fontSize: '18px', padding: '16px 40px', borderRadius: '12px',
              textDecoration: 'none',
            }}
          >
            Add to Chrome — Free
        ...
        <footer style={{
          ...
          <a href="https://eddy.delivery" style={{ color: '#64748b', textDecoration: 'none' }}>eddy.delivery</a>
          {' · '}
          {config.footerSlogan}
        </footer>
      </main>
