...
function extractCart(): CartDetection | null {
  ...
  // Extract real fees from Uber Eats checkout
  const realFees = extractRealFees();
  return { platform: 'ubereats', restaurantName, items, pageUrl: location.href, ...(realFees ? { realFees } : {}) };
}
...
function extractRealFees(): RealFees | null {
  ...
  const feeMap: Record<string, keyof RealFees> = {
    ...
    'taxes': 'tax', 'tax': 'tax', 'estimated tax': 'tax', 'total': 'total',
    'order total': 'total', 'small order fee': 'smallOrderFee',
  };

  // UE uses cart summary with labeled rows
  const containers = document.querySelectorAll('[class*="cart"], [class*="Cart"], [class*="checkout"], [class*="Checkout"], [data-testid*="cart"], [data-testid*="checkout"]');
  for (const container of containers) {
    const els = container.querySelectorAll('span, div, li');
