# Chrome Web Store Listing

## Extension Name
Eddy — Save on Food Delivery

## Short Description (132 chars max)
Compare total costs across DoorDash, Uber Eats, Grubhub & direct ordering. Save $5-15 per order. Free.

## Detailed Description
Stop overpaying for food delivery. The same order costs different amounts on every platform — and the hidden fees add up fast.

Eddy automatically compares prices when you browse DoorDash, Uber Eats, or Grubhub:

🔍 AUTOMATIC DETECTION — Add items to your cart on any delivery app. Eddy detects your order and compares prices in the background.

💰 TOTAL COST COMPARISON — Not just menu prices. We compare the FULL cost: food + service fee + delivery fee + small order fee + tax. This is what you actually pay.

🏪 DIRECT ORDERING — Many restaurants offer delivery through their own website at 10-20% lower prices with no service fee. Eddy finds these for you.

🔗 ONE-CLICK SWITCH — Found a cheaper option? Click to open the order on that platform. Deep links take you straight to the restaurant.

📊 REAL SAVINGS — Average savings of $5-15 per order. For a family ordering delivery twice a week, that's $500-1,500/year.

HOW IT WORKS:
1. Browse DoorDash, Uber Eats, or Grubhub like normal
2. Add items to your cart
3. Click the Eddy icon to see price comparisons
4. Switch to the cheapest option in one click

CURRENTLY AVAILABLE IN:
• Austin, TX
• Washington, DC
• More cities coming soon!

FREE FOREVER — No premium tier, no data selling, no tricks.

## Category
Shopping

## Language
English

## Privacy Policy URL
https://eddy.vercel.app/privacy

## Website
https://eddy.vercel.app

## Screenshots Needed (1280x800 or 640x400)
1. Popup showing price comparison (Chipotle example)
2. Badge showing savings amount on toolbar
3. Landing page hero
4. Fee breakdown comparison
5. One-click switch deep link

## Promotional Images
- Small tile: 440x280
- Marquee: 1400x560
